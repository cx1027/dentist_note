...
async function generateSummary() {
  try {
      ...
      .then(response => response.json())
      .then(summary => {
          console.log("Summary response:", summary.summary);
          const summaryText = summary.summary;
          const formattedText = jsonToHtmlText(summaryText);
          document.getElementById('summary-content').innerHTML = formattedText;
...
async function generateTemplateSummary() {
    try {
        // Get both text values
        const templateText = document.getElementById('template').innerText;  // Use .value for textarea
        const transcriptionText = document.getElementById('transcription').innerText;
        
        console.log("Template text:", templateText);
        console.log("Transcription text:", transcriptionText);

        // Send POST request with both texts
...
function jsonToHtmlText(jsonData) {
    try {
        const data = typeof jsonData === 'string' ? JSON.parse(jsonData) : jsonData;
        let formattedText = '<div class="summary-content">';
        
        // Process main sections
        for (const [key, value] of Object.entries(data)) {
            if (key !== 'Teeth_Description') {
                const formattedKey = key.replace(/_/g, ' ');
                formattedText += `<div class="section">`;
                formattedText += `<div class="title"><strong>${formattedKey}:</strong></div>`;
                ...
                if (typeof value === 'string' && value.includes('.')) {
                    ...
                    points.forEach((point, index) => {
                        formattedText += `<div class="point">- ${point}</div>`;
                    });
                } else {
                    formattedText += `<div class="point">- ${value}</div>`;
                }
                formattedText += `</div>`;
            }
        }
        
        // Process Teeth Description separately with tooth numbers
        if (data.Teeth_Description) {
            formattedText += `<div class="section">`;
            formattedText += `<div class="title"><strong>Teeth Description:</strong></div>`;
            for (const [tooth, description] of Object.entries(data.Teeth_Description)) {
                formattedText += `<div class="point">- Tooth ${tooth}: ${description}</div>`;
            }
            formattedText += `</div>`;
        }
...
function updateSummaryDisplay(summaryText) {
    try {
        ...
        document.getElementById('summary-output').innerText = 'Error displaying summary';
    }
}
